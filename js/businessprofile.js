...
function validateDispatchAddress() {
    clearBusinessProfileError();
    var flag = true;
    var warehouse_code1 = $("#warehouse_code1").val();
    var disp_off_land1 = $("#disp_off_land1").val();
    var disp_address1 = $("#disp_address1").val();
    var disp_area1 = $("#disp_area1").val();
    var disp_city1 = $("#disp_city1").val();
    var disp_state1 = $("#disp_state1").val();
    var disp_district1 = $("#disp_district1").val();
    var disp_pincode1 = $("#disp_pincode1").val();

    if (warehouse_code1.trim().length = 0) {
        $("#warehouse_code_error1").html("Warehouse Code Should not be empty");
        flag = false;
    }

    if (disp_off_land1.trim().length > 0) {
        if (isNaN(disp_off_land1)) {
            flag = false;
            $("#disp_off_land_error1").html("Office Landline should only contain numbers");
        } else {
            if (disp_off_land1.trim().length >= 10 && disp_off_land1.length <= 12) {

            } else {
                flag = false;
                $("#disp_off_land_error1").html("Office Landline should be between 10 to 12 digits");
            }
        }
    } else {
        $("#disp_off_land_error1").html("Office Landline cannot be empty");
        flag = false;
    }

    if (disp_address1.trim().length < 10) {
        $("#disp_address_error1").html("Address should have at least 10 characters");
        flag = false;
    }
    if (disp_area1.trim().length < 10) {
        $("#disp_area_error1").html("Area should have at least 10 characters");
        flag = false;
    }

    if (disp_city1.trim().length < 3) {
        $("#disp_city_error1").html("Area should have at least 3 characters");
        flag = false;
    }

    if (disp_state1 === "Select State") {
        $("#disp_state_error1").html("Please Select State");
        flag = false;
    }

    if (disp_district1 === "no") {
        $("#disp_district_error1").html("Please Select District");
        flag = false;
    }

    var pin_regex = /^\d{6}$/;
    if (!pin_regex.test(disp_pincode1)) {
        $("#disp_pincode_error1").html("Pincode should have 6 digits");
        flag = false;
    }
